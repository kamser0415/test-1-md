...
**차이점**  
직접 도커 서버에 배포를 하는게 아니라 Ansible 서버에 관리할 파일을 복사하고 
Ansible을 활용해서 해당 파일을 관리를 합니다.  
  
Ansible 서버에는 관리 대상 클라이언트 서버를 텍스트 문자로 관리하고,  
기본은 `/etc/ansible/hosts`이지만, 경로만 일치하면 어떤 파일이든 타겟 대상으로 지정할 수 있습니다.  
...
ansible을 사용하는 이유는 사용하지 않는다면 기존 컨테이너를 지우는 과정을 
command를 통해서 삭제를 해야했습니다. 
ansible은 스크립트 파일을 통해서 모듈과 같이 사용해서 제어할 수 있습니다.
